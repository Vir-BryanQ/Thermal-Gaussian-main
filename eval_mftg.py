...
def perform_sampling(args):
    ...
    thgs_dir = os.path.join("outputs", "MFTG", dataset_name)
    os.makedirs(thgs_dir, exist_ok=True)

    ts = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    rgb_model_path = os.path.join(thgs_dir, ts)

    training_command = (f"python train_MFTG.py -s {args.dataset_path} -m {rgb_model_path} --step 1")
    ...
    checkpoint_path = os.path.join(rgb_model_path, 'chkpnt30000.pth')
    points_color_path = os.path.join(rgb_model_path, 'point_cloud_color')
    ...
    for i in tqdm(range(len(train_lists))):
        ...
        training_command = (f"python train_MFTG.py -s {args.dataset_path} -m {model_path} --train_file {train_file} --step 2 --start_checkpoint {checkpoint_path} --no_report")
        print(training_command)
        ...
        print(training_time)
        training_time = f"{float(training_time):.1f}"

        process.wait()

        preallocate_vmem()

        print(f'Copy {points_color_path} to {model_path}')
        dst_path = os.path.join(model_path, 'point_cloud_color')
        shutil.copytree(points_color_path, dst_path)

        print(f'Copy {checkpoint_path} to {model_path}')
        dst_file = os.path.join(model_path, 'chkpnt_rgb_30000.pth')
        shutil.copy2(checkpoint_path, dst_file)  
        ...
        ssim_rgb = eval_results[f'ours_{training_iterations}']['color SSIM']
        psnr_rgb = eval_results[f'ours_{training_iterations}']['color PSNR']
        lpips_rgb = eval_results[f'ours_{training_iterations}']['color LPIPS']
        ssim_th = eval_results[f'ours_{training_iterations}']['thermal SSIM']
        psnr_th = eval_results[f'ours_{training_iterations}']['thermal PSNR']
        lpips_th = eval_results[f'ours_{training_iterations}']['thermal LPIPS']
