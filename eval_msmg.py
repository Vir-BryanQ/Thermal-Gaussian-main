...
def perform_sampling(args):
    ...
    thgs_dir = os.path.join("outputs", "MSMG", dataset_name)
    ...
    for i in tqdm(range(len(train_lists))):
        ...
        training_command = (f"python train_MSMG.py -s {args.dataset_path} -m {model_path} --train_file {train_file} --iterations {training_iterations} --no_report")
